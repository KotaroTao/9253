...
- **口コミ誘導はオプション機能** — アンケート完了後にGoogle口コミページへのリンクを全患者に一律表示する機能をクリニック単位で有効/無効切替可能。回答結果による出し分けは行わず、景品表示法・医療広告ガイドラインに完全準拠
...
| 設定 | ✅ | クリニック名、日次目標、営業日数/週、定休日、臨時休診日、Google口コミ誘導（オプション） |
| Google口コミ誘導 | ✅ | オプション機能。アンケート完了後に全患者一律でGoogle口コミリンクを表示。スコアによる出し分けなし。景品表示法・医療広告ガイドライン準拠。クリニック設定で有効/無効切替 |
...
## 設計判断の記録
- **Google口コミ誘導はオプション機能**: アンケート完了後にGoogle口コミページへのリンクを表示する機能を実装。景品表示法・医療広告ガイドライン完全準拠のため、(1) 回答結果やスコアに関わらず全患者に一律表示、(2) インセンティブなし、(3) 投稿は任意である旨を明示。`Clinic.settings` JSONB の `googleReviewEnabled`（boolean）と `googleReviewUrl`（string）で制御。デフォルト無効
...
- 医療広告ガイドライン準拠: Google口コミ誘導はオプション機能として提供。回答結果やスコアによる表示の出し分けを行わず全患者に一律表示、インセンティブなし、投稿任意を明示し、景品表示法・医療広告ガイドラインに完全準拠
- 患者アンケートは匿名・任意
