...
- **口コミ・LINE誘導はオプション機能** — アンケート完了後の誘導先を3択で設定可能: ①アンケートのみで終了、②Google口コミに誘導、③医院LINEに誘導。②③選択時は全患者に一律表示（回答結果による出し分けなし）で景品表示法・医療広告ガイドライン完全準拠。別途、医院ホームページリンクも表示可能（②③選択時は控えめ表示）
...
| 設定 | ✅ | クリニック名、日次目標、営業日数/週、定休日、臨時休診日、アンケート完了後誘導（3択+医院HP） |
| アンケート完了後誘導 | ✅ | 3択オプション: ①アンケートのみ終了 ②Google口コミ誘導 ③LINE誘導。全患者一律表示、スコアによる出し分けなし。医院HPリンクは独立表示可能（②③選択時は控えめ）。景品表示法・医療広告ガイドライン準拠 |
...
## 設計判断の記録
- **アンケート完了後誘導は3択+医院HPの構成**: `Clinic.settings` JSONB の `postSurveyAction`（"none" | "google_review" | "line"）で誘導先を制御。`googleReviewUrl`（Google口コミ）、`lineUrl`（LINE公式）、`clinicHomepageUrl`（医院HP）をそれぞれ保存。②③選択時は誘導CTAを目立たせ、医院HPリンクは控えめに表示。景品表示法・医療広告ガイドライン完全準拠: 回答結果やスコアによる出し分けなし、インセンティブなし、投稿/友だち追加は任意と明示。デフォルトは「none」（アンケートのみで終了）
