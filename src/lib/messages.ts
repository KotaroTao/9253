export const messages = {
  ...
  improvementActions: {
    ...
    tips: [
      ...
      "改善アクションは「開始時スコア → 現在スコア」で効果を数値化できます。スコアが動いたらチームで共有し、成功体験を積み上げましょう。",
      "まずは「待ち時間」と「受付対応」から着手するのが王道。この2つは患者が最初に体験するため、第一印象を大きく左右します。",
      "改善施策は最低1ヶ月は続けてからスコアを確認しましょう。短期間での判断は回答数が少なく、本当の効果が見えません。",
      "完了したアクションも定期的に振り返りましょう。一度改善した項目でも、意識が薄れるとスコアが戻ることがあります。",
      "スタッフ全員が改善アクションの内容を知っていることが大切です。朝礼やミーティングで「今やっていること」を共有しましょう。",
      "患者の自由記述コメント（患者の声）は改善のヒントの宝庫です。回答一覧も合わせて確認しましょう。",
      "スコアが4.5以上の質問は「維持する」ことが大切。無理に改善するより、低スコアの項目にリソースを集中する方が効果的です。",
      "「痛みへの配慮」のスコア改善には、治療前の声かけを1つ追加するだけでも効果があります。小さな一歩から始めましょう。",
      "改善アクションの「中止」は失敗ではありません。効果がない施策を早めにやめて、別のアプローチに切り替える判断も重要です。",
      "同じカテゴリの改善アクションを複数同時に実施すると、どの施策が効いたか分かりにくくなります。1つずつ試すのが効果測定のコツです。",
      "曜日×時間帯のヒートマップで満足度が低い時間帯を特定し、その時間帯に絞った改善施策を立てると効率的です。",
      "「費用説明」のスコアが低い場合、治療前の見積もり提示を徹底するだけで劇的に改善するケースが多いです。",
      "改善アクションの説明欄には「誰が・いつ・何を」を具体的に書くと、スタッフ間で認識がずれにくくなります。",
      "初診と再診でスコアの傾向は異なります。満足度レポートのテンプレート別スコアを確認し、ターゲットを明確にしましょう。",
      "スコアが上がったら、スタッフダッシュボードの日次目標と合わせてチームを褒めましょう。モチベーションが改善の持続力を生みます。",
      "「紹介意向」のスコアは他の全設問の総合結果です。個別の設問スコアを底上げすることが、紹介意向の改善につながります。",
      "経営レポートと改善アクションを組み合わせると、「この施策で自費率が上がった」など経営効果の可視化が可能になります。",
      "実施履歴にメモを残しておくと、後から振り返る際に「何を変えたらスコアが動いたか」が明確になります。",
      "改善は終わりのないプロセスです。1つ完了したら次の低スコア項目に取り組む、この繰り返しが医院全体の患者体験を高めます。",
    ],
  },
  staffLeaderboard: {
    title: "アンケート回収件数",
  ...
  monthlyMetrics: {
    ...
    revenuePerReceipt: "レセプト単価",
    avgVisitsPerPatient: "平均通院回数",
    laborCostRatio: "人件費率",
    revenuePerStaff: "スタッフ1人あたり売上",
